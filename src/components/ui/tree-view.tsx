...
export interface TreeViewProps {
  ...
  enableModifierMultiSelect?: boolean;
}
...
const defaultIconMap: TreeViewIconMap = {
  file: <HugeiconsIcon icon={PackageIcon} className="h-4 w-4 text-red-600" />,
  folder: <HugeiconsIcon icon={Folder01Icon} className="h-4 w-4 text-primary/80" />,
};
...
function TreeItem({
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // In checkbox-driven mode, clicking row/label should toggle checkbox state.
    if (showAccessRights && !enableModifierMultiSelect) {
      if (onAccessChange) {
    ...
    if (enableModifierMultiSelect && e.shiftKey && lastSelectedId.current !== null) {
    ...
    } else if (enableModifierMultiSelect && (e.ctrlKey || e.metaKey)) {
                  ...
                  >
                    ...
                    >
                      <Button variant="ghost" size="icon" className="h-6 w-6">
                        <motion.div
                          initial={false}
                          animate={{ rotate: isOpen ? 90 : dir === "rtl" ? 180 : 0 }}
                    ...
                    >
                      {item.children?.map((child) => (
                        <TreeItem
                          ...
                          enableModifierMultiSelect={enableModifierMultiSelect}
                        />
                      ))}
                    </motion.div>
                  </CollapsibleContent>
...
export function TreeView({
  ...
  enableModifierMultiSelect = true,
}: TreeViewProps) {
  ...
  const getItemsForIds = useCallback((ids: Set<string>): TreeViewItem[] => {
  ...
  const resolvedCheckboxLabels = useMemo(
    () => ({
      check: checkboxLabels?.check ?? (t.has("actions.check") ? t("actions.check") : "Check"),
      uncheck:
        checkboxLabels?.uncheck ?? (t.has("actions.uncheck") ? t("actions.uncheck") : "Uncheck"),
    }),
    [checkboxLabels?.check, checkboxLabels?.uncheck, t],
  );

  const updateSelection = useCallback((next: Set<string>) => {
    if (setsEqual(selectedIdsRef.current, next)) return;
    selectedIdsRef.current = next;
    setSelectedIds(next);
    onSelectionChangeRef.current?.(getItemsForIds(next));
  }, [getItemsForIds]);

  useEffect(() => {
    const next = new Set(defaultSelectedIds);
    if (setsEqual(selectedIdsRef.current, next)) return;
    selectedIdsRef.current = next;
    setSelectedIds(next);
  ...
  }, [clearSelectionOnClickAway, updateSelection]);
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!isDragging) {
        if (distance > DRAG_THRESHOLD) {
          ...
          if (!enableModifierMultiSelect || (!e.shiftKey && !e.ctrlKey && !e.metaKey)) {
            updateSelection(new Set());
            lastSelectedId.current = null;
          }
        }
      ...
      const keepPreviousSelection =
        enableModifierMultiSelect && (e.shiftKey || e.ctrlKey || e.metaKey);
      const newSelection = new Set(keepPreviousSelection ? Array.from(selectedIds) : []);
    ...
    [isDragging, dragStart, selectedIds, dragStartPosition, updateSelection, enableModifierMultiSelect]
  );
  ...
  return (
    <div className={cn("w-full", containerClassName)}>
      <div
        ref={treeRef}
        className="bg-background rounded-lg border space-y-2 w-full relative"
      >
        <AnimatePresence mode="wait">
          {selectedIds.size > 0 ? (
            ...
            >
              ...
              >
                <HugeiconsIcon icon={Cancel01Icon} className="h-4 w-4 me-2" />
            ...
            >
              <div className="relative flex-1">
                <HugeiconsIcon
                  icon={Search01Icon}
                  className="absolute end-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"
                />
                <Input
              ...
              {showExpandAll && (
                <div className="flex gap-2 shrink-0">
                  ...
                  >
                    <HugeiconsIcon icon={ArrowDown01Icon} className="h-4 w-4 me-1" />
                    Expand All
                  </Button>
                  <Button
                    variant="ghost"
                  ...
                  >
                    <HugeiconsIcon
                      icon={dir === "rtl" ? ArrowLeft01Icon : ArrowRight01Icon}
